...
import {
  getCounterPercent,
  readBonusesStats,
  trackedBonusCivilizations,
  trackedBonuses,
} from './bonusesStats.js'
...
export function BonusesTrainerStatsPage() {
  const [stats, setStats] = useState(() => readBonusesStats())
  const [sortOrder, setSortOrder] = useState('asc')

  useEffect(() => {
    const onStorage = () => {
      setStats(readBonusesStats())
    }
  ...
  const civilizationRows = useMemo(
    () => buildRows(trackedBonusCivilizations, stats.civs, sortOrder),
    [sortOrder, stats.civs],
  )
  const bonusRows = useMemo(
    () => buildRows(trackedBonuses, stats.bonuses, sortOrder),
    [sortOrder, stats.bonuses],
  )
      ...
      <section className="panel wide">
        <div className="top-bar trainer-nav-top-bar">
          <div className="trainer-nav-actions">
            <Link to="/bonuses-trainer" className="trainer-nav-button" onClick={playMenuClickSound}>
              Back to Bonuses Trainer
            </Link>
        ...
        <h1 className="section-title">Bonuses Trainer Stats</h1>
        <p className="section-subtitle">Track your civ and team bonus recognition.</p>
        ...
        <div className="trainer-stats-columns">
          ...
          <section className="trainer-stats-column">
            <h2 className="trainer-stats-column-title">Bonuses</h2>
            <div className="trainer-stats-list">
              {bonusRows.length === 0 ? (
                <p className="section-subtitle">No bonus stats yet.</p>
              ) : (
                bonusRows.map((row) => (
                  <article key={row.id} className="trainer-stats-row">
                    <div className="trainer-stats-meta">
                      ...
                      <span className="trainer-stats-name">
                        {row.name}
                        <span className="trainer-stats-tag">{row.typeLabel}</span>
                      </span>
                    </div>
                    <div className="trainer-stats-bar-wrap">
                      <p className="trainer-stats-percent-label">
