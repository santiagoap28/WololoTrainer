...
import { getCounterPercent, readCrownsStats, trackedCrownCivilizations, trackedCrownTechs } from './crownsStats.js'
...
export function CrownsTrainerStatsPage() {
  const [stats, setStats] = useState(() => readCrownsStats())
  const [sortOrder, setSortOrder] = useState('asc')

  useEffect(() => {
    const onStorage = () => {
      setStats(readCrownsStats())
    }
  ...
  const civilizationRows = useMemo(
    () => buildRows(trackedCrownCivilizations, stats.civs, sortOrder),
    [sortOrder, stats.civs],
  )
  const crownRows = useMemo(
    () => buildRows(trackedCrownTechs, stats.crowns, sortOrder),
    [sortOrder, stats.crowns],
  )
      ...
      <section className="panel wide">
        <div className="top-bar trainer-nav-top-bar">
          <div className="trainer-nav-actions">
            <Link to="/crowns-trainer" className="trainer-nav-button" onClick={playMenuClickSound}>
              Back to Crowns Trainer
            </Link>
        ...
        <h1 className="section-title">Crowns Trainer Stats</h1>
        <p className="section-subtitle">Track your unique-tech crown recognition.</p>
        ...
        <div className="trainer-stats-columns">
          ...
          <section className="trainer-stats-column">
            <h2 className="trainer-stats-column-title">Crown Techs</h2>
            <div className="trainer-stats-list">
              {crownRows.length === 0 ? (
                <p className="section-subtitle">No crown tech stats yet.</p>
              ) : (
                crownRows.map((row) => (
                  <article key={row.id} className="trainer-stats-row">
                    <div className="trainer-stats-meta">
                      ...
                      <span className="trainer-stats-name">
                        {row.name}
                        <span className="trainer-stats-tag">
                          {row.crownType === 'silver' ? 'Silver Crown' : 'Gold Crown'}
                        </span>
                      </span>
                    </div>
                    <div className="trainer-stats-bar-wrap">
